...
class Polygon:
    ...
    def create_dart(self, idx):
        ...
        point_opposites = [[y for k, y in enumerate(x) if y != idx or (
            k != 0 and k != 1)] for x in point_edges]
        for j, point_opposite in enumerate(point_opposites):
            point_opposites[j].append(Polygon.compute_edge_angle(
                self.points[idx], self.points[point_opposite[0]]))
    ...
    def draw(points=[], l=0.05):
        ...
        boundary_edges = [x for x in sorted(boundary_edges, key=itemgetter(2))[
            ::-1] if x[2] > l]
        ...
        while len(boundary_edges) > 0:
            ...
            if is_regular_bool:
                ...
                edge.sort()
                edge.append(Polygon.distance(
                    polygon.points[edge[0]], polygon.points[edge[1]]))
                ...
                # Update edges
                polygon.edges = [x for x in polygon.edges if not (
                    x[0] == boundary_edge[0] and x[1] == boundary_edge[1])]
        ...
        while len(ordered_chain) != nb_boundary_edges:
            # Find vertex edge
            edge = [[i, x] for i, x in enumerate(
                boundary_edges_remain) if x[0] == vertex or x[1] == vertex][0]
